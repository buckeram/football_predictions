...
def make_predictions(event):
    ...
    for league in leagues:
        ...
        home_clf, draw_clf, away_clf, best_home_score, best_draw_score, best_away_score, home_best_fname, draw_best_fname, away_best_fname, train_summary = train_classifiers(past_league_matches, league)
        mail_message += train_summary
        ...
        mail_message += predict(past_league_matches, league, classifiers, next_matches)
    ...
    response = ses.send_email(
        ...
        Message={
            ...
            'Subject': {
                'Charset': 'UTF-8',
                'Data': "Football predictions for {}".format(datetime.now()),
            }
        },
...
def train_classifiers(data, league):
    ...
    """ % (league, home_best_fname, best_home_score, away_best_fname, best_away_score, draw_best_fname, best_draw_score)
    training_summary = textwrap.dedent(training_summary)
...
def predict(data, league, classifiers, matches, csv=False):
    league_data = get_league_data(data, league)
    ...
    for match in matches:
        ...
        home = classifiers['home_clf'].predict([[home_diff]])
        away = classifiers['away_clf'].predict([[away_diff]])
        draw = classifiers['draw_clf'].predict([[draw_diff]])
        if csv:
            output += "%s,%s,%s,%.2f,%.2f,%.2f" % (league, home_team, away_team, home, draw, away)
        else:
            prediction = """
            %s: %s
            --------------------------------------------------
            ...
            output += textwrap.dedent(prediction)
